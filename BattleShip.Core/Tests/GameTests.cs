namespace Tests
{
    ...
    public class GameTests
    {
        ...
        public void TakeShot_ShotOnExactTarget_ReturnsHit()
        {
            ...
            game = game.Apply(new PlayerLocationUpdated(game.Id, game.NextPlayerToTakeShot.Email, new GeoCoordinate(0.0, 0.0)));
            game = game.Apply(new PlayerLocationUpdated(game.Id, game.Opponent.Email, new GeoCoordinate(5.0, 5.0)));

            var result = game.Apply(new ShotTaken(game.Id, game.NextPlayerToTakeShot, game.Opponent.Location));

            Assert.AreEqual(ShotResult.Hit, result.LastShot.ShotResult);
        }
...
//
        ...
        {
            var player1 = new Player("A@b.com", "A", null, playerTargetZoneRadius);
            var player2 = new Player("B@b.com", "B", null, playerTargetZoneRadius);

            var game = new Game("Test", player1, player2, null, playerTargetZoneRadius, shotBlastRadius);
